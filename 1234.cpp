...
private:
	...
	void calculate_Hs()
	{
		...
		for (int n = 0; n < NUM_DISK - 1; ++n) {
			if (vvi_Board[n].size() > 1) {  //���� ��ǥ Ÿ�� �ƴѰ� ���� 2�� �̻� �ִٸ�
	...
	void Move_Add_NewNode(int j, int i, std::vector<Hanoi_node>& v_board)
	{
		...
		temp.vvi_Board[j].emplace_back(vvi_Board[i][vvi_Board[i].size() - 1]); //�ٸ� Ÿ�� ���� Ÿ���� ����� ��� ����
		temp.vvi_Board[i].erase(temp.vvi_Board[i].begin() + vvi_Board[i].size() - 1); // ���� Ÿ�� ����� ��� ���
	...
	vector<Hanoi_node> possible_NextNode() {
		...
		for (int i = 0; i < NUM_DISK; ++i) {
			for (int j = 0; j < NUM_DISK; ++j) {
				...
				else if (vvi_Board[i][vvi_Board[i].size() - 1] < vvi_Board[j][vvi_Board[j].size() - 1]) { //�ٸ� Ÿ���� �ֻ��� ������ �۴ٸ�
					Move_Add_NewNode(j, i, v_board);
				}
			}
		}
